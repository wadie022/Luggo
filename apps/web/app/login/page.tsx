...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setApiErrors(data);
        throw new Error(data.detail || "Connexion impossible. Vérifie tes identifiants.");
      }
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900 flex items-center justify-center px-4">
      <div className="w-full max-w-xl rounded-3xl border border-slate-200 bg-white shadow-sm p-6">
        ...
        <h1 className="mt-6 text-3xl font-extrabold tracking-tight">Connexion</h1>
        <p className="mt-2 text-slate-600 text-sm">
          Connecte-toi pour accéder automatiquement à ton espace.
        </p>
